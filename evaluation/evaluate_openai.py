...
def create_vectorstore_from_df(df, persist_path="../data/chroma_eval_openai"):
    if os.path.exists(persist_path) and len(os.listdir(persist_path)) > 0:
        return Chroma(embedding_function=OpenAIEmbeddings(model="text-embedding-3-large", openai_api_key=os.getenv("OPENAI_API_KEY")), persist_directory=persist_path)
    documents = [
